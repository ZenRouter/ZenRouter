...
export default function DeepSeekTuiToolCard({
  ...
  useEffect(() => {
    ...
    if (initialStatus) {
      ...
      (async () => {
        if (!cancelled) setDeepseekStatus(nextStatus);
      })();
    }
    return () => { cancelled = true; };
  }, [initialStatus]);
    ...
    if (!deepseekStatus) {
      (async () => {
        ...
        try {
          const res = await fetch(ENDPOINT);
          const data = await res.json();
          if (!cancelled) setDeepseekStatus(data);
        } catch (error) {
          if (!cancelled) setDeepseekStatus({ installed: false, error: error.message });
        } finally {
  ...
  useEffect(() => {
    ...
    if (deepseekStatus?.installed && !hasInitializedModel.current) {
      ...
      if (openaiSection?.model) {
        const nextModel = openaiSection.model;
        (async () => {
          ...
          {!checking && deepseekStatus?.installed && (
            <>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!deepseekStatus?.hasZenRouter} loading={restoring}>
