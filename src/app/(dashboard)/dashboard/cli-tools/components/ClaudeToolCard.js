...
export default function ClaudeToolCard({
  ...
  useEffect(() => {
    ...
    const nextTokens = v || "";
    (async () => {
      if (!cancelled) setAutoCompactWindow(nextTokens);
    })();
    return () => { cancelled = true; };
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      // Get key from dropdown, fallback to first key or sk_zenrouter for localhost
          ...
          {!checkingClaude && claudeStatus?.installed && (
            <>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!claudeStatus?.hasZenRouter} loading={restoring}>
