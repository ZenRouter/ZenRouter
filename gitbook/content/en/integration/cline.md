...
Integrate ZenRoute with Cline VSCode extension to route your AI requests through ZenRoute's intelligent routing system.
...
Set the base URL to your ZenRoute endpoint:
...
**Steps:**
1. In the **Base URL** field, enter your ZenRoute endpoint
...
3. The key should start with `sk-zenroute-`

### 5. Select Model

1. In the **Model** dropdown, you can either:
   - Select from available models (if Cline auto-detects them)
   - Manually enter the model name from your ZenRoute configuration
...
4. Cline will use ZenRoute to process your request

### Code Generation

1. Ask Cline to generate code: "Create a React component for a login form"
2. Cline will generate code using ZenRoute
...
3. Get AI-powered explanations through ZenRoute

### File Operations

1. Ask Cline to create, modify, or delete files
2. Cline will use ZenRoute to understand context and make changes
...
2. Verify your ZenRoute instance is running and healthy
3. Try reloading VSCode window (Cmd/Ctrl + Shift + P → "Reload Window")
4. Check ZenRoute logs for any errors
...
## Integration with ZenRoute Features

### Model Routing

ZenRoute automatically routes your requests to the best available provider based on:
- Model availability
...
If a provider fails, ZenRoute automatically falls back to alternative providers configured in your dashboard.

### Usage Tracking

Monitor your Cline usage through ZenRoute dashboard:
- Total requests
- Token usage
- Cost per model
