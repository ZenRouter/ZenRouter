...
Tích hợp ZenRoute với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của ZenRoute.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- ZenRoute đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint ZenRoute:
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng ZenRoute để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua ZenRoute
...
3. Nhận giải thích AI qua ZenRoute

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng ZenRoute để hiểu context và thực hiện thay đổi
...
1. Xác minh ZenRoute đang chạy: `curl http://localhost:20128/health`
...
2. Xác minh ZenRoute instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs ZenRoute để xem lỗi
...
1. Trong Cline settings, đặt Base URL: `https://zenroute.dev`
2. Đảm bảo bạn đã cấu hình API key trong ZenRoute cloud dashboard
...
## Tích hợp với Tính năng ZenRoute

### Định tuyến Model

ZenRoute tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, ZenRoute tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua ZenRoute dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
