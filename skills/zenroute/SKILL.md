---
name: zenroute
description: Entry point for ZenRoute — local/remote AI gateway with OpenAI-compatible REST for chat, image, TTS, embeddings, web search, web fetch. Use when the user mentions ZenRoute, ZENROUTE_URL, or wants AI without writing provider boilerplate. This skill covers setup + indexes capability skills; fetch the relevant capability SKILL.md from the URLs below when needed.
---
...
```bash
export ZENROUTE_URL="http://localhost:20128"      # or VPS / tunnel URL
export ZENROUTE_KEY="sk-..."                      # from Dashboard → Keys (only if requireApiKey=true)
```

All requests: `${ZENROUTE_URL}/v1/...` with header `Authorization: Bearer ${ZENROUTE_KEY}` (omit if auth disabled).
...
```bash
curl $ZENROUTE_URL/v1/models                  # chat/LLM (default)
curl $ZENROUTE_URL/v1/models/image            # image-gen
curl $ZENROUTE_URL/v1/models/tts              # text-to-speech
curl $ZENROUTE_URL/v1/models/embedding        # embeddings
curl $ZENROUTE_URL/v1/models/web              # web search + fetch (entries have `kind` field)
curl $ZENROUTE_URL/v1/models/stt              # speech-to-text
curl $ZENROUTE_URL/v1/models/image-to-text    # vision
```
...
- 401 → set/refresh `ZENROUTE_KEY` (Dashboard → Keys)
