...
describe.each([
  ...
  it("preserves assistant prefill when explicitly requested", () => {
    ...
    ], { "x-zenrouter-assistant-prefill": "preserve" });
...
describe("defaultClaudeToolType", () => {
  it("defaults missing tool type to custom", () => {
    const tools = [{ name: "search", description: "web search" }];
    const normalized = defaultClaudeToolType(tools);
    expect(normalized[0].type).toBe("custom");
  });

  it("keeps explicit tool type intact", () => {
    const tools = [{ type: "web_search_20250101", name: "search" }];
    const normalized = defaultClaudeToolType(tools);
    expect(normalized[0].type).toBe("web_search_20250101");
  });
});
